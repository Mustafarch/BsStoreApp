...
namespace Services
{
    public class AuthenticationManager : IAuthenticationService
    {
        ...
        public async Task<IdentityResult> RegisterUser(UserForRegistrationDto userForRegistrationDto)
        {
            ...
            if (result.Succeeded)
            {
                await _userManager.AddToRolesAsync(user, userForRegistrationDto.Roles);   //user, userForRegistrationDto daki rolleri aldı.
            }
            return result;
        }
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token) //ClaimsPrincipal kullanıcı bilgilerini istemek için kullanılıyor.
        {
            var jwtSettings = _configuration.GetSection("JwtSettings");
